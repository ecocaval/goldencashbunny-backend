...
CREATE TABLE space_table_column (
  id UUID NOT NULL,
   ...
   deleted BOOLEAN DEFAULT FALSE NOT NULL,
   name VARCHAR(255) NOT NULL,
   column_reference INTEGER NOT NULL,
   space_table_id UUID,
   column_type VARCHAR(255) NOT NULL,
   CONSTRAINT pk_spacetablecolumn PRIMARY KEY (id)
);

CREATE TABLE space_table_column_row (
  id UUID NOT NULL,
   creation_date TIMESTAMP WITHOUT TIME ZONE NOT NULL,
   last_modified_date TIMESTAMP WITHOUT TIME ZONE,
   deleted BOOLEAN DEFAULT FALSE NOT NULL,
   ...
   CONSTRAINT pk_spacetablecolumnrow PRIMARY KEY (id)
);
...
ALTER TABLE space_table_column_row ADD CONSTRAINT FK_SPACETABLECOLUMNROW_ON_SPACE_TABLE_COLUMN FOREIGN KEY (space_table_column_id) REFERENCES space_table_column (id);
